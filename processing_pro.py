    ...
    def run(self):
        ...
        while True:
            ...
            if len(corners) > 0:
                ...
                ids = ids.flatten()
                if initial_count == 0 and len(ids) != len(set(ids)):   # Determine if duplicate markerIDs are caught in the initial state
                    print("Error 300: If the dice aruco marker was initially recognized")
                ...
                for (markerCorner, markerID) in zip(corners, ids):
                    ...
                    if markerID == 2:
                        ...
                        self.hamster_position[markerID] = (cX, cY, height, w, topLeft, topRight, bottomLeft, bottomRight)
                    ...
                    if markerID != 2 and initial_count != 0:         # Need to calculate the dice position after the the initial state
                        ...
                        if dice_markerid == 7:
                            if abs(self.board_position[markerID][0] - cX) >= 10 or abs(self.board_position[markerID][1] - cY) >= 10 or abs(self.board_position[markerID][2] - height) >= 10:
                                if dice_markerid not in self.dice_position:
                                    self.dice_position[markerID] = (cX, cY, height)  # Dice is thrown -> update the dice position
                        ...
                        if dice_markerid in self.dice_position:
                            ...
                            if self.degree >= 15:
                                ...
                                if self.distance < 20:
                                    hamster.stop()
                                    dice_markerid = 7      # Initializing to roll the next dice
                    self.marker_positions[markerID].append((cX, cY, height))  # Save location information for all recognized aruco markers
            ...
            for i in range(7):
                if i == 2:
                    ...
                    if not any(self.marker_positions[i]):       # If there are no values in that list
                        ...
                        self.board_position.clear()
                        break
                    if self.marker_positions[i][0][2] <= 55:     # Distinguished by the height of the aruco marker on board
                        print("Error 400: Instead of the board aruco marker, another aruco marker is stored")
                        # Initialization operation
                        initial_count = 0
                        for j in range(7):
                            self.marker_positions[j].clear()
